...
int testapprun(instance_data_t *inst, int message)
{
    switch (inst->testAppState)
    {
        ...
        case TA_SLEEP_DONE :
        {
            ...
            //dwt_setleds(3); // TBD: Used for debugging
        ...
        case TA_TXFINAL_WAIT_SEND :
            {
                ...
                if(instancesenddlypacket(inst, DWT_START_TX_DELAYED))
                {
                    ...
                    if(inst->mode == TAG)
                    {
                        ...
                        inst->instToSleep = TRUE ;  //bug fix ... tag needs to go to sleep if this TX fails : Decawave changes
#endif
...
#ifdef NORDIC_NRF52840_BOARD
    instance_data[instance].pollTx2FinalTxDelayAnc = convertmicrosectodevicetimeu (delayus + 100);
#else
